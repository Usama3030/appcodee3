...
const [error, setError] = useState(false)
  ...
  const addProfessor = (e) => {
    e.preventDefault();
    if(fname.length==0||lname.length==0||pswd.length==3||subject.length==0||department.length==0||add.length==0||city.length==0||email.length==0){
      setError(true)
    }
...
required/>
{error&&pswd.length<=3?
<label style={{color:"red"}}>Password must not be less then 4 digit</label>:""}
</div>
<div class="col-md-4">
...
<div class="col-12">
<button type="submit" class="btn btn-primary" onClick={addProfessor}>Submit</button>
</div>
